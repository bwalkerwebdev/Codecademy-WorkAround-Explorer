...
function updateResults() {
  ...
  const formatSalByRole = formatNumber(averageSalaryByRole);
  const formatAvgSal = formatNumber(averageSalaryByCompany);
  const formatSalary = formatNumber(salary);
  const formatIndustryAvg = formatNumber(industryAverageSalary)

  // Render them to the screen.
  document.getElementById(
    "salarySelected"
  ).innerText = `The salary for ${role}s at ${company} is \$${formatSalary}`;
  document.getElementById(
    "salaryAverageByRole"
  ).innerText = `The industry average salary for ${role} positions is \$${formatSalByRole}`;
  document.getElementById(
    "salaryAverageByCompany"
  ).innerText = `The average salary at ${company} is \$${formatAvgSal}`;
  document.getElementById(
    "salaryAverageIndustry"
  ).innerText = `The average salary in the Tech industry is \$${formatIndustryAvg}`;
}
